...
@Service
public class TruckTicketingEngine implements TicketEngine {
    @Override
    public Ticket createTicket(Vehicle vehicle, ParkingSpot parkingSpot) {
        Ticket ticket = new Ticket(TicketIdGenerator.generateRandomTicketId(),vehicle,parkingSpot, LocalDateTime.now(),null, TRUCK_FARE,TRUCK_ADDITIONAL_FARE_PER_HOUR, TicketStatus.OPEN);
